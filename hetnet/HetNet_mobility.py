...
class BS():
    ...
    def __init__(self, sim, BStype, id_x, cell_color, x_pos, y_pos, cell_id, config_p):
        ...
        if self.get_type() == "SBS":
            self.obs_len = 160#12 #10
        else:
            self.obs_len = int(160 * self.mbs_interval / 10)
        ...
        self.max_antenna_ul = 100 #4
        self.max_antenna_dl = 100 #4
    ...
    def gen_bs_agent(self):
        ...
        network_spec=[
            ...
            dict(type='dense', size=self.obs_len/2, activation='softplus'),
            dict(type='dense', size=self.obs_len/4, activation='relu')
        ]
...
class UE():
    ...
    def gen_random_data_ue(self, time_step):
        ...
        for _ in range(self.sim.ul_random[self.get_id()][time_step]):
            self.gen_data(self.inside_BS, data_size, "ul")
        for _ in range(self.sim.dl_random[self.get_id()][time_step]):
            self.gen_data(self.inside_BS, data_size, "dl")
    ...
    def find_BS(self, old_bs):
        ...
        print("xy_pos:"+str(self.get_xy()))
        for bs in self.sim.BS_list:
            if (bs.get_type() == "SBS"):
                distance = calc_d(bs.get_xy(), self.get_xy())
                if (distance < (sbs_radius /2)):
                    print("find_BS:"+str(bs.get_name()))
                    return(bs)
        ...
        for bs in self.sim.BS_list:
            if (bs.get_type() == "MBS"):
                distance = calc_d(bs.get_xy(), self.get_xy())
                print("bs_pos:"+str(bs.get_xy())+"ue_pos:"+str(self.get_xy())+"distance:"+str(distance)+","+str(self.get_name()))
        ...
        for bs in mbs_list:
            ...
            if len(ue_inside) < min_inside:
                ...
                min_bs = bs                    
        print("find_BS:"+str(min_bs.get_name()))
        return(min_bs)
    ...
    def move(self):
        print("move:"+str(self.get_xy())+" ,"+str(self.direction)+" ,"+str(self.get_name()))
    ...
    def moving_up(self, n): #0
        ...
        if self.y_axis >= self.y_lim_r:
            flag = True
        if (pre_y_axis == self.y_lim_r) or ((((pre_y_axis - self.y_lim_l) % self.y_grid) != 0) and (((pre_y_axis - self.y_lim_l) // self.y_grid) != ((self.y_axis - self.y_lim_l) // self.y_grid))):
            self.coner(flag)
    ...
    def moving_down(self, n): #2
        ...
        if self.y_axis <= self.y_lim_l:
            flag = True
        if (pre_y_axis == self.y_lim_l) or ((((pre_y_axis - self.y_lim_l) % self.y_grid) != 0) and (((pre_y_axis - self.y_lim_l) // self.y_grid) != ((self.y_axis - self.y_lim_l) // self.y_grid))):
            self.coner(flag)
            tmp_y_axis = self.y_axis
            print("pre_y_axis:"+str(pre_y_axis))
            self.y_axis = ((pre_y_axis - self.y_lim_l)// self.y_grid) * self.y_grid + self.y_lim_l
    ...
    def moving_right(self, n): #1
        ...
        if self.x_axis >= self.x_lim_r:
            flag = True
        if (pre_x_axis == self.x_lim_r) or ((((pre_x_axis - self.x_lim_l) % self.x_grid) != 0) and (((pre_x_axis - self.x_lim_l) // self.x_grid) != ((self.x_axis - self.x_lim_l) // self.x_grid))):
            self.coner(flag)
    ...
    def moving_left(self, n): #3
        ...
        if (pre_x_axis == self.x_lim_l) or ((((pre_x_axis - self.x_lim_l) % self.x_grid) != 0) and (((pre_x_axis - self.x_lim_l) // self.x_grid) != ((self.x_axis - self.x_lim_l) // self.x_grid))):
            self.coner(flag)
            tmp_x_axis = self.x_axis
            self.x_axis = ((pre_x_axis - self.x_lim_l) // self.x_grid) * self.x_grid + self.x_lim_l
    ...
    def coner(self, flag):
        ...
        if self.turn_right_prob >= prob:
            ...
            self.direction = (self.direction - 1) % 4
        print("flag:"+str(flag)+", "+str(prob)+", "+str(pre_direction)+", "+str(self.direction)+" ,"+str(self.get_name()))
...
#######################
    ...
    def execute_DRL_mobility(self):
        ...
        for i in self.BS_list:
            if (i.get_type() == "SBS") and (i.skip_flag == 0):
