...
def check_grid(pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l):
...
def find_xy1(x, y, mbs_pos, sbs_pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l):
    ...
    while (check_distance(answer, mbs_pos, sbs_pos) == False) and (check_grid(answer, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l) == False):
        answer = find_xy_inside_triangel(p0x, p0y, p1x, p1y, p2x, p2y)
    
    return(answer)

def find_xy2(x, y, mbs_pos, sbs_pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l):
...
def find_xy3(x, y, mbs_pos, sbs_pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l):
...
def find_xy_c(p0x, p0y, radius, site_pos, sbs_pos):
    ...
    while answer == False:
        ...
        answer2 = minimum_distance(px, py, site_pos, sbs_pos)

        answer = answer and answer2
    
    return([px, py])

def find_xy_c_mobility(x_lim_l, y_lim_l, x_lim_r, y_lim_r):

    px = random.randint(x_lim_l, x_lim_r)
    py = random.randint(y_lim_l, y_lim_r)

    return([px, py])

def minimum_distance(px, py, mbs_pos, sbs_pos):
    for i in mbs_pos:
        if calc_d([px, py], [i[0], i[1]]) < 35:
            return(False)
    for i in sbs_pos:
        if calc_d([px, py], [i[0], i[1]]) < 10:
            return(False)
    return(True)
...
for i in site_pos:
    ...
    for id_mbs in range(num_mbs_per_site):
        ...
        for _ in range(num_sbs_per_mbs):
            if id_mbs % 3 == 0:
                ans = find_xy1(i[0], i[1], site_pos, sbs_pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l)
            if id_mbs % 3 == 1:
                ans = find_xy2(i[0], i[1], site_pos, sbs_pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l)
            if id_mbs % 3 == 2:
                ans = find_xy3(i[0], i[1], site_pos, sbs_pos, x_grid, y_grid, x_lim_r, y_lim_r, x_lim_l, y_lim_l)
            cell_id = cell_id + 1
...
for i in site_pos:
    ...
    for id_mbs in range(num_mbs_per_site):
        ...
        #mbs_id = mbs_id + 1
        for _ in range(UE_per_mbs):
            #print("D2:"+str(mbs_list[mbs_ind].get_name()))
            #ueans = find_xy_c(i[0], i[1], mbs_radius, site_pos, sbs_pos)
            ueans = find_xy_c_mobility(x_lim_l, y_lim_l, x_lim_r, y_lim_r)
            sim.add_ue("Ped", ue_id, cell_color, ueans[0], ueans[1], mbs_list[mbs_ind])
            ue_id = ue_id + 1
        mbs_ind = mbs_ind + 1
        ...
        for _ in range(num_sbs_per_mbs):
            ...
            #sbs_id = sbs_id + 1
            for _ in range(UE_per_sbs):
                #print("D4:"+str(sbs_list[sbs_ind].get_name()))
                #ueans = find_xy_c(sbs_pos[sbs_ind][0], sbs_pos[sbs_ind][1], sbs_radius, site_pos, sbs_pos)
                ueans = find_xy_c_mobility(x_lim_l, y_lim_l, x_lim_r, y_lim_r)
                sim.add_ue("Ped", ue_id, cell_color, ueans[0], ueans[1], sbs_list[sbs_ind])
                ue_id = ue_id + 1
            sbs_ind = sbs_ind + 1
