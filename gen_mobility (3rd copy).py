...
for i in range(x_lim_l, x_lim_r+x_grid, x_grid):
    for j in range(y_lim_l, y_lim_r+y_grid, y_grid):
        ...
        #for _ in range(ue_random[ind]):
            ...
            if r_rand < prob_ped:
                ue = UE_mobility(i, j, random.randrange(ped_speed-5, ped_speed+5))
            ...
            #    UE_list.append(ue)            
                ...
                ue = UE_mobility(i, j, random.randrange(veh_speed-10, veh_speed+10))
...
f = open("data/position"+str(prob_ped).replace('.', '')+"_"+args[2]+".csv", 'w')
writer = csv.writer(f)
